    ...
    if (req.file) {
      ...
      src.on("end", async () => {
        try {
          ...
          await product.save();
          return res.json({ msg: "Save product success", data: product });
        } catch (err) {
...
const index = async (req, res, next) => {
  try {
    ...
    let products = await Product.find(criteria)
      ...
      .populate("category")
      .populate("tags");
    return res.json({ msg: "Get products", count, data: products });
  } catch (err) {
    next(err);
  }
};
    ...
    if (req.file) {
      ...
      src.on("end", async () => {
        try {
          ...
          return res.json({ msg: "Update product success", data: product });
        } catch (err) {
          fs.unlinkSync(targetPath);
          handleErr(err, res);
          next(err);
